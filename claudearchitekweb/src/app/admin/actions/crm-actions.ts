...
export async function createLeadAction(formData: FormData) {
  ...
  logActivity("lead", lead.id, `Entered manually by ${user.name}`, "system", user.id);
...
export async function updateLeadAction(formData: FormData) {
  ...
  db.update(schema.leads)
    ...
    .run();
  if (before.estimatedValue !== data.estimatedValue) logActivity("lead", id, `Estimated value set to ${data.estimatedValue ?? "—"} ${data.currency}`, "system", user.id);
...
export async function updateLeadStatusAction(formData: FormData) {
  ...
  if (lead.status !== status) {
    ...
    logActivity("lead", id, `Status: ${lead.status} → ${status}`, "status", user.id);
  }
  revalidateEntity("lead", id);
}
...
export async function markLeadLostAction(formData: FormData) {
  ...
  logActivity("lead", id, `Marked lost${reason ? `: ${reason}` : ""}`, "status", user.id);
  revalidateEntity("lead", id);
}
...
export async function convertLeadAction(formData: FormData) {
  ...
  const title = fopt(formData, "title", 160) ?? undefined;
  const r = convertLead(id, { userId: user.id, projectType, title });
  revalidatePath("/admin/leads");
...
export async function createClientAction(formData: FormData) {
  ...
  logActivity("client", id, `Created by ${user.name}`, "system", user.id);
  if (data.companyId) logActivity("company", data.companyId, `Contact added: ${data.firstName} ${data.lastName ?? ""}`.trim(), "system", user.id);
  revalidatePath("/admin/clients");
...
export async function updateClientAction(formData: FormData) {
  ...
  if (before.status !== data.status) logActivity("client", id, `Status: ${before.status} → ${data.status}`, "status", user.id);
...
export async function createCompanyAction(formData: FormData) {
  ...
  logActivity("company", id, `Created by ${user.name}`, "system", user.id);
  revalidatePath("/admin/companies");
...
export async function updateCompanyAction(formData: FormData) {
  ...
  if (before.status !== data.status) logActivity("company", id, `Status: ${before.status} → ${data.status}`, "status", user.id);
