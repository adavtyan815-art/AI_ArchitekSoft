      ...
      {rows.map((r) => (
        <li key={r.key} className="py-2">
          <div className="flex items-center justify-between text-[13.5px]">
            ...
            <span className="num ml-3 text-fg">{r.value}</span>
          </div>
...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-5 border-y border-line py-2">
        {RANGES.map((d) => (
          <Link key={d} href={`/admin/analytics?days=${d}`} className={cn("border-b-2 pt-1 pb-0.5 font-mono text-[12px] tracking-[0.02em] transition-colors", days === d ? "border-fg text-fg" : "border-transparent text-muted hover:text-fg")}>
            {d} {L.days}
          </Link>
        ))}
      </div>
                ...
                <tbody>
                  {site.topPages.map((p) => (
                    <tr key={p.path ?? "?"}>
                      ...
                      <td data-label={L.table.views} className="num">{p.count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {a.clientPages.topLinks.map((l) => (
                    <tr key={l.id}>
                      <td data-label={L.pages.page}>
                        <Link href="/admin/pages" className="font-semibold text-fg hover:text-accent">{l.title ?? l.slug}</Link>
                      </td>
                      <td data-label={L.pages.project} className="text-xs text-fg-2">{l.projectCode} · {l.projectTitle}</td>
                      <td data-label={L.table.views} className="num">{l.views}</td>
                      <td data-label={L.pages.lastView} className="font-mono text-[12px] whitespace-nowrap text-muted">{fmtYerevan(l.lastViewedAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.2fr]">
        <Panel title={L.panels.perPlatform}>
          ...
          <div className="mt-4">
            <div className="mb-2 font-mono text-[10px] tracking-[0.12em] text-muted uppercase">{L.panels.postsByStatus}</div>
